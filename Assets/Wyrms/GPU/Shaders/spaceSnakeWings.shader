  ...
  SubShader{
    ...
    Pass{
      ...
      float3 doBGCol( float3 pos , float id ){
        pos = pos * 2;
        float n1 = noise(  pos * 3+ id * 100  + float3(0,_Time.y,0));
...
float n  = noise( 40  * float3( v.uv.x , v.uv.y * .3 , _Time.x + 200 * v.id ));
       	float distFromCenter = abs( v.uv.y - .5 ) * 2;
       	float eyeMatch  = dot( normalize(fNorm), -normalize(v.eye) );
       	col = lerp( normalize(fNorm) * .5 + .5 , fRefl * .5  + .5 , v.id) * cubeCol ;
